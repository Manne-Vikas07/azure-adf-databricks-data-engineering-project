...
df_src = spark.sql('''
select distinct(Branch_ID) as Branch_ID, BranchName from parquet.`abfss://silver@vikasproject2storage.dfs.core.windows.net/carsales`
''')
...
if spark.catalog.tableExists("cars_catalog.gold.dim_branch"):
    ...
    delta_tbl = DeltaTable.forPath(
        spark,
        "abfss://gold@vikasproject2storage.dfs.core.windows.net/dim_branch"
    )
...
# Initial Run
    ...
    df_final.write.format("delta") \
        .mode("overwrite") \
        .option("path", "abfss://gold@vikasproject2storage.dfs.core.windows.net/dim_branch") \
